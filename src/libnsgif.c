...
// drwr: Pebble doesn't provide calloc() or realloc(), so we
// reimplement them here.
void *local_calloc(size_t count, size_t size) {
  char *ptr = (char *)malloc(count * size);
  memset(ptr, 0, count * size);
  return ptr;
}

void *local_realloc(void *old_ptr, size_t size) {
...
#define calloc(count, size) local_calloc((count), (size))
#define realloc(old_ptr, size) local_realloc((old_ptr), (size))
    ...
    if (!clear_image) {
        ...
        */
        ...
        for (y = 0; y < height; y++) {
            ...
            frame_scanline = frame_data + offset_x + (decode_y * row_stride);
            int current_bit = 0x80;
				...
				if (burst_bytes > 0) {
					...
					while (burst_bytes-- > 0) {
						...
						if (((gif->frames[frame].transparency) &&
                          ...
                          if (colour_table[colour] != 0) {
                            (*frame_scanline) |= current_bit;
                          }
                        }
		...
		*/
		if (gif->frames[frame].disposal_method == GIF_FRAME_CLEAR) {
			for (y = 0; y < height; y++) {
              ...
              frame_scanline = frame_data + offset_x + ((offset_y + y) * row_stride);
              memset(frame_scanline, 0, row_stride);
              /*
				if (gif->frames[frame].transparency)
